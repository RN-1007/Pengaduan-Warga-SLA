...
const RippleCursor: React.FC<RippleCursorProps> = ({
  ...
  const handleMouseMove = (e: MouseEvent): void => {
    if (window.innerWidth < 768) return; // Hide on mobile devices
  ...
  return (
    <div className='fixed top-0 left-0 w-screen h-screen pointer-events-none overflow-hidden z-[9999] hidden md:block'>
      {ripples.map((ripple) => (
        <div
          key={ripple.id}
